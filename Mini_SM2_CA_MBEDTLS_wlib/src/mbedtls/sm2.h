 ...
 *                  or a MBEDTLS_ERR_ECP_XXX or MBEDTLS_MPI_XXX error code
 */
int mbedtls_sm2_sign_det( mbedtls_ecp_group *grp, mbedtls_mpi *r, mbedtls_mpi *s,
                    const mbedtls_mpi *d, const unsigned char *buf, size_t blen,
 ...
 *                  or a MBEDTLS_ERR_ECP_XXX or MBEDTLS_MPI_XXX error code
 */
int mbedtls_sm2_verify_hash( mbedtls_ecp_group *grp,
 ...
 *                  (Not thread-safe to use same context in multiple threads)
 *
 * \deprecated      Superseded by mbedtls_sm2_write_signature() in 2.0.0
 *
 * \param ctx       ECDSA context
 * \param hash      Message hash
 * \param hlen      Length of hash
 ...
 *                  MBEDTLS_ERR_ASN1_XXX error code
 */
int mbedtls_sm2_write_signature_det( mbedtls_sm2_context *ctx,
                               const unsigned char *hash, size_t hlen,
                               unsigned char *sig, size_t *slen,
...
int mbedtls_sm2_compute_za(unsigned char *pbID, int iIDLen,
	mbedtls_ecp_group *grp, mbedtls_ecp_point *Q,
	unsigned char *za);

int mbedtls_sm2_compute_e(mbedtls_md_type_t md_alg, unsigned char *id, int id_len,
	unsigned char *data, int data_len, mbedtls_ecp_keypair *pk, unsigned char *e);
